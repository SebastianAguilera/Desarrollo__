...
@universidad_bp.route('/universidad', methods=['POST'])
    def crear():
        ...
        universidad.sigla = escape(universidad.sigla)  # Escapar caracteres especiales
...
@validate_with(universidad_mapping)
def actualizar_universidad(id:int):
    universidad = universidad.mapping.load(request.get_json())
    universidad.nombre = escape(universidad.nombre)
    universidad.sigla = escape(universidad.sigla)  # Escapar caracteres especiales
    UniversidadService.actualizar_universidad(id, universidad), 200

        return jsonify("Universidad actualizada exitosamente"), 200 #200 significa que se actualizo exitosamente 
